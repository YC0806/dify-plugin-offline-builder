...
class DifyPluginRepackager:
    ...
    def repackage(self, package_path: str):
        ...
        try:
            ...
            # Step 1: Download build dependencies first (setuptools, wheel)
            ...
            subprocess.run(build_deps_cmd, check=True)

            # Step 2: Use pip wheel to build all dependencies as wheels (including source distributions)
            print("Building all dependencies as wheels (including source distributions)...")
            pip_wheel_cmd = [
                "pip", "wheel",
            ...
            subprocess.run(pip_wheel_cmd, check=True)

            # Step 3: Verify all files in wheels directory are .whl files
            print("Verifying all packages are in wheel format...")
            non_wheel_files = [f for f in wheels_dir.iterdir() if f.suffix not in ['.whl']]
            if non_wheel_files:
                print(f"Warning: Found non-wheel files: {[f.name for f in non_wheel_files]}")
                print("These files may require building in the target environment.")
            else:
                print("All dependencies are pre-built as wheels!")
